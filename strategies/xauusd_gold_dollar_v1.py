#!/usr/bin/env python3
"""XAUUSD Gold-Dollar research strategy v4.6.
...
v4.6 blocks Friday first London hour (07-08 UTC) when real spread/ATR is elevated vs the 20-day same-hour median AND volume is thin vs same-hour median. No fake spread/volume.
"""
...
def thursday_london_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Thursday 07-08 UTC has elevated spread/ATR vs same-hour 20-day median.

    Never invents spread. If ATR is zero the ratio is NaN and the bar is not blocked.
    """
    hour = index.hour
    wd = index.weekday
    in_win = (wd == 3) & (hour >= p.london_open_hour) & (hour < p.london_open_end_hour)
    atr_safe = atr.replace(0, np.nan)
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(
        window=max(p.thursday_london_spread_atr_lookback * 24, p.thursday_london_spread_atr_lookback),
        min_periods=5,
    ).median()
    high_ratio = ratio >= (p.thursday_london_spread_atr_med_mult * med_ratio)
    out = in_win & high_ratio.fillna(False)
    return out.astype(bool)


def thursday_london_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    """True when Thursday 07-08 UTC has elevated spread/ATR vs same-hour median AND thin volume.
    ...
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(window=max(p.thursday_london_spread_atr_lookback * 24, p.thursday_london_spread_atr_lookback), min_periods=5).median()
    vol_win = volume.where(in_win)
    med_vol = vol_win.rolling(window=max(p.thursday_london_vol_lookback * 24, p.thursday_london_vol_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.thursday_london_spread_atr_med_mult * med_ratio)
    thin = volume < (p.thursday_london_thin_vol_ratio * med_vol)
    out = in_win & high_ratio.fillna(False) & thin.fillna(False)
    return out.astype(bool)


def friday_london_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Friday 07-08 UTC has elevated spread/ATR vs same-hour 20-day median.

    Never invents spread. If ATR is zero the ratio is NaN and the bar is not blocked.
    """
    hour = index.hour
    wd = index.weekday
    in_win = (wd == 4) & (hour >= p.london_open_hour) & (hour < p.london_open_end_hour)
    atr_safe = atr.replace(0, np.nan)
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(
        window=max(p.friday_london_spread_atr_lookback * 24, p.friday_london_spread_atr_lookback),
        min_periods=5,
    ).median()
    high_ratio = ratio >= (p.friday_london_spread_atr_med_mult * med_ratio)
    out = in_win & high_ratio.fillna(False)
    return out.astype(bool)


def friday_london_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    """True when Friday 07-08 UTC has elevated spread/ATR vs same-hour median AND thin volume.
    ...
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(window=max(p.friday_london_spread_atr_lookback * 24, p.friday_london_spread_atr_lookback), min_periods=5).median()
    vol_win = volume.where(in_win)
    med_vol = vol_win.rolling(window=max(p.friday_london_vol_lookback * 24, p.friday_london_vol_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.friday_london_spread_atr_med_mult * med_ratio)
    thin = volume < (p.friday_london_thin_vol_ratio * med_vol)
    out = in_win & high_ratio.fillna(False) & thin.fillna(False)
    return out.astype(bool)
