#!/usr/bin/env python3
"""XAUUSD Gold-Dollar research strategy v5.3.
...
v5.3 blocks Wednesday last session hour (19-20 UTC) when real spread/ATR is elevated
...
def monday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Monday 19-20 UTC has elevated spread/ATR vs same-hour 20-day median.
    ...
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(
        window=max(p.monday_session_close_spread_atr_lookback * 24, p.monday_session_close_spread_atr_lookback),
        min_periods=5,
    ).median()
...
def monday_session_close_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    """True when Monday 19-20 UTC has elevated spread/ATR vs same-hour median AND thin volume.
...
def tuesday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Tuesday 19-20 UTC has elevated spread/ATR vs same-hour 20-day median.
    ...
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(
        window=max(p.tuesday_session_close_spread_atr_lookback * 24, p.tuesday_session_close_spread_atr_lookback),
        min_periods=5,
    ).median()
...
def wednesday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Wednesday 19-20 UTC has elevated spread/ATR vs same-hour 20-day median.
    ...
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(
        window=max(p.wednesday_session_close_spread_atr_lookback * 24, p.wednesday_session_close_spread_atr_lookback),
        min_periods=5,
    ).median()
