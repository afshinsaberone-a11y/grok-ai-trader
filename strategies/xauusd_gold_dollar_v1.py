...
def monday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    hour = index.hour
    wd = index.weekday
    in_win = (wd == 0) & (hour >= p.session_close_hour) & (hour < p.session_close_end_hour)
    atr_safe = atr.replace(0, np.nan)
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(window=max(p.monday_session_close_spread_atr_lookback * 24, p.monday_session_close_spread_atr_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.monday_session_close_spread_atr_med_mult * med_ratio)
    out = in_win & high_ratio.fillna(False)
    return out.astype(bool)


def monday_session_close_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    out = pd.Series(False, index=index)
    if volume is None:
        return out
    hour = index.hour
    ...
    med_ratio = same_hour_ratio.rolling(window=max(p.monday_session_close_spread_atr_lookback * 24, p.monday_session_close_spread_atr_lookback), min_periods=5).median()
    vol_win = volume.where(in_win)
    med_vol = vol_win.rolling(window=max(p.monday_session_close_vol_lookback * 24, p.monday_session_close_vol_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.monday_session_close_spread_atr_med_mult * med_ratio)
    thin = volume < (p.monday_session_close_thin_vol_ratio * med_vol)
    out = in_win & high_ratio.fillna(False) & thin.fillna(False)
    return out.astype(bool)


def tuesday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    hour = index.hour
    wd = index.weekday
...
def wednesday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    hour = index.hour
    wd = index.weekday
    in_win = (wd == 2) & (hour >= p.session_close_hour) & (hour < p.session_close_end_hour)
    atr_safe = atr.replace(0, np.nan)
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(window=max(p.wednesday_session_close_spread_atr_lookback * 24, p.wednesday_session_close_spread_atr_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.wednesday_session_close_spread_atr_med_mult * med_ratio)
    out = in_win & high_ratio.fillna(False)
    return out.astype(bool)


def wednesday_session_close_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    """True when Wednesday 19-20 UTC has elevated spread/ATR vs same-hour median AND thin volume.
    ...
    med_ratio = same_hour_ratio.rolling(window=max(p.wednesday_session_close_spread_atr_lookback * 24, p.wednesday_session_close_spread_atr_lookback), min_periods=5).median()
    vol_win = volume.where(in_win)
    med_vol = vol_win.rolling(window=max(p.wednesday_session_close_vol_lookback * 24, p.wednesday_session_close_vol_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.wednesday_session_close_spread_atr_med_mult * med_ratio)
    thin = volume < (p.wednesday_session_close_thin_vol_ratio * med_vol)
    out = in_win & high_ratio.fillna(False) & thin.fillna(False)
    return out.astype(bool)


def thursday_session_close_spread_atr_med_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, p: GoldParams) -> pd.Series:
    """True when Thursday 19-20 UTC has elevated spread/ATR vs same-hour 20-day median.

    Never invents spread. If ATR is zero the ratio is NaN and the bar is not blocked.
    """
    hour = index.hour
    wd = index.weekday
    in_win = (wd == 3) & (hour >= p.session_close_hour) & (hour < p.session_close_end_hour)
    atr_safe = atr.replace(0, np.nan)
    ratio = spread_used / atr_safe
    same_hour_ratio = ratio.where(in_win)
    med_ratio = same_hour_ratio.rolling(window=max(p.thursday_session_close_spread_atr_lookback * 24, p.thursday_session_close_spread_atr_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.thursday_session_close_spread_atr_med_mult * med_ratio)
    out = in_win & high_ratio.fillna(False)
    return out.astype(bool)


def thursday_session_close_spread_atr_med_thin_block(index: pd.DatetimeIndex, spread_used: pd.Series, atr: pd.Series, volume: pd.Series | None, p: GoldParams) -> pd.Series:
    """True when Thursday 19-20 UTC has elevated spread/ATR vs same-hour median AND thin volume.
    ...
    med_ratio = same_hour_ratio.rolling(window=max(p.thursday_session_close_spread_atr_lookback * 24, p.thursday_session_close_spread_atr_lookback), min_periods=5).median()
    vol_win = volume.where(in_win)
    med_vol = vol_win.rolling(window=max(p.thursday_session_close_vol_lookback * 24, p.thursday_session_close_vol_lookback), min_periods=5).median()
    high_ratio = ratio >= (p.thursday_session_close_spread_atr_med_mult * med_ratio)
