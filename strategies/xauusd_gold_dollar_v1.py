...
class XAUUSDGoldDollarV1:
    ...
    def prepare(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["monday_london_wide_block"] = False
        out["monday_london_thin_wide_block"] = False
        if isinstance(out.index, pd.DatetimeIndex):
            ...
            monday_london = (out["weekday"] == self.p.monday_gap_weekday) & london_hour
            mon_lon_wide = out["spread_used"] >= (self.p.monday_london_wide_spread_atr * out["atr"])
            out["monday_london_wide_block"] = monday_london & mon_lon_wide.fillna(False)
            ...
            if vol_col is not None:
                ...
                mon_lon_hour_vol = vol.where(monday_london)
                mon_lon_hour_sum = mon_lon_hour_vol.groupby(day).transform("sum")
                mon_lon_day_hour_sum = mon_lon_hour_vol.groupby(day).sum()
                mon_lon_hour_med = mon_lon_day_hour_sum.rolling(self.p.monday_london_vol_lookback, min_periods=5).median()
                mon_lon_hour_med_mapped = pd.Series(day, index=out.index).map(mon_lon_hour_med)
                thin_mon_lon = mon_lon_hour_sum < (self.p.monday_london_thin_vol_ratio * mon_lon_hour_med_mapped)
                out["monday_london_thin_wide_block"] = monday_london & thin_mon_lon.fillna(False) & mon_lon_wide.fillna(False)
            out["session_ok"] = (
                ...
                & (~out["monday_london_wide_block"])
                & (~out["monday_london_thin_wide_block"])
            )
    ...
    def signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        monday_london_ok = ~d["monday_london_wide_block"].fillna(False)
        monday_london_and_ok = ~d["monday_london_thin_wide_block"].fillna(False)
        base = (
            squeeze
            & vol_ok
            & d["session_ok"]
        ...
        d.attrs["monday_london_wide_blocks"] = int(d["monday_london_wide_block"].fillna(False).sum())
        d.attrs["monday_london_thin_wide_blocks"] = int(d["monday_london_thin_wide_block"].fillna(False).sum())
        return d
