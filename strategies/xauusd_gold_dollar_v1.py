...
class XAUUSDGoldDollarV1:
    ...
    def prepare(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(out.index, pd.DatetimeIndex):
            ...
            out["session_ok"] = (out["hour"] >= self.p.session_start) & (out["hour"] < flatten_from)
            out["flatten_now"] = out["hour"] >= flatten_from
    ...
    def backtest(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(1, len(d)):
            ...
            if pos == 1:
                ...
                if low <= stop:
                    ...
                    closed = True
                    time_stop_closes += 1
                elif bool(row.get("flatten_now", False)):
                    r_now = (close - entry) / init_risk if init_risk else 0
                    realized = r_now if not half_done else 0.5 * r_now
                    closed = True
                    session_flatten_closes += 1
            else:
                if init_risk > 0:
        ...
        return {
            ...
            "session_flatten_closes": session_flatten_closes,
            "flatten_lead_hours": self.p.flatten_lead_hours,
