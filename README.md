# grok-ai-trader

ربات‌ها و تحقیقات استراتژی فارکس ساخته‌شده توسط **Grok**.

## آخرین تحقیق
**شناسه:** GRK-FX-2026-009 (کاتالوگ امتیازدهی + رژیم‌سوئیچ v2.50 + لوپ رفع ایراد)

### فایل‌های کلیدی
- `research/BEST_STRATEGIES_CATALOG_2026.md`
- `research/EA_AUDIT_LOOP_009.md`
- `research/project_repair_loop.py`
- `strategies/GRK_REGIME_SWITCH.md`
- `ea/GRK_Hybrid_Regime_EA.mq5` نسخه ۲.۴۰ (چک‌لیست استاتیک پاس)

```bash
python research/project_repair_loop.py
```
