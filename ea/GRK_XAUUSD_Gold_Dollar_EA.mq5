...
//|     شناسه: GRK-XAUUSD-GOLD-DOLLAR-001  |  نسخه: 5.30             |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   Print("GRK XAUUSD Gold-Dollar EA v5.30 ready");
...
bool FridaySessionCloseSpreadAtrMedThinBlock()
{
   ...
   if(gt.hour < SessionCloseHour || gt.hour >= SessionCloseEndHour)
      return false;
   if(!FridaySessionCloseSpreadAtrMedBlock())
      return false;
   long vol_now = iVolume(_Symbol,PERIOD_H1,0);
   if(vol_now<=0)
      return false;
   double sample[];
   ArrayResize(sample, FridaySessionCloseVolLookback);
   int n=0;
   ...
   if(med<=0.0)
      return false;
   bool thin = ((double)vol_now < FridaySessionCloseThinVolRatio * med);
   return thin;
}
...
bool MondaySessionCloseSpreadAtrMedBlock()
{
   ...
   double ratio_now = spread / atr[0];
   double sample[];
   ArrayResize(sample, MondaySessionCloseSpreadAtrLookback);
   int n=0;
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(point<=0.0)
      return false;
...
bool MondaySessionCloseSpreadAtrMedThinBlock()
{
   ...
   if(gt.hour < SessionCloseHour || gt.hour >= SessionCloseEndHour)
      return false;
   if(!MondaySessionCloseSpreadAtrMedBlock())
      return false;
   long vol_now = iVolume(_Symbol,PERIOD_H1,0);
   if(vol_now<=0)
      return false;
   double sample[];
   ArrayResize(sample, MondaySessionCloseVolLookback);
   int n=0;
   ...
   if(med<=0.0)
      return false;
   bool thin = ((double)vol_now < MondaySessionCloseThinVolRatio * med);
   return thin;
}
...
bool TuesdaySessionCloseSpreadAtrMedBlock()
{
   ...
   double ratio_now = spread / atr[0];
   double sample[];
   ArrayResize(sample, TuesdaySessionCloseSpreadAtrLookback);
   int n=0;
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(point<=0.0)
      return false;
