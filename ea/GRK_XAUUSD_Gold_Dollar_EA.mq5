...
//|     شناسه: GRK-XAUUSD-GOLD-DOLLAR-001  |  نسخه: 6.90             |
//+------------------------------------------------------------------+
...
input double AsiaReopenThinVolRatio=0.70;
input int    AsiaReopenVolLookback=20;
input double MondayAsiaReopenSpreadAtrMedMult=1.50;
input int    MondayAsiaReopenSpreadAtrLookback=20;
input double MondayAsiaReopenThinVolRatio=0.70;
input int    MondayAsiaReopenVolLookback=20;
...
int OnInit()
{
   ...
   Print("GRK XAUUSD Gold-Dollar EA v6.90 ready");
...
bool AsiaReopenSpreadAtrMedThinBlock()
{
   if(!AsiaReopenSpreadAtrMedBlock()) return false;
   MqlDateTime gt; TimeToStruct(TimeGMT(), gt);
   if(gt.hour < AsiaReopenStartHour || gt.hour >= AsiaReopenEndHour) return false;
   long vol_now = iVolume(_Symbol, PERIOD_H1, 0);
   if(vol_now <= 0) return false;
   double sample[]; ArrayResize(sample, AsiaReopenVolLookback);
   int n=0;
   for(int i=1; i<=AsiaReopenVolLookback*30 && n<AsiaReopenVolLookback; i++)
   {
      datetime bar_time = iTime(_Symbol,PERIOD_H1,i);
      if(bar_time==0) continue;
      MqlDateTime bt; TimeToStruct(bar_time, bt);
   ...
   double med = sample[n/2];
   if(med<=0.0) return false;
   return ((double)vol_now < AsiaReopenThinVolRatio * med);
}

bool MondayAsiaReopenSpreadAtrMedBlock()
{
   MqlDateTime gt; TimeToStruct(TimeGMT(), gt);
   ...
   if(spread<=0.0) return false;
   double ratio_now = spread / atr[0];
   double sample[]; ArrayResize(sample, MondayAsiaReopenSpreadAtrLookback);
   int n=0;
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(point<=0.0) return false;
   for(int i=1; i<=MondayAsiaReopenSpreadAtrLookback*30 && n<MondayAsiaReopenSpreadAtrLookback; i++)
   {
      datetime bar_time = iTime(_Symbol,PERIOD_H1,i);
      if(bar_time==0) continue;
      MqlDateTime bt; TimeToStruct(bar_time, bt);
   ...
   double med = sample[n/2];
   if(med<=0.0) return false;
   return (ratio_now >= MondayAsiaReopenSpreadAtrMedMult * med);
}

bool MondayAsiaReopenSpreadAtrMedThinBlock()
{
   if(!MondayAsiaReopenSpreadAtrMedBlock()) return false;
   MqlDateTime gt; TimeToStruct(TimeGMT(), gt);
   if(gt.day_of_week != 1) return false;
   if(gt.hour < AsiaReopenStartHour || gt.hour >= AsiaReopenEndHour) return false;
   long vol_now = iVolume(_Symbol, PERIOD_H1, 0);
   if(vol_now <= 0) return false;
   double sample[]; ArrayResize(sample, MondayAsiaReopenVolLookback);
   int n=0;
   for(int i=1; i<=MondayAsiaReopenVolLookback*30 && n<MondayAsiaReopenVolLookback; i++)
   {
      datetime bar_time = iTime(_Symbol,PERIOD_H1,i);
      if(bar_time==0) continue;
      MqlDateTime bt; TimeToStruct(bar_time, bt);
