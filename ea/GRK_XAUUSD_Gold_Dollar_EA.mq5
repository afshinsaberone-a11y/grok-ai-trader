...
//|     شناسه: GRK-XAUUSD-GOLD-DOLLAR-001  |  نسخه: 3.70             |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   lastDay=TimeCurrent();
   Print("GRK XAUUSD Gold-Dollar EA v3.70 ready");
...
bool SessionCloseThinWideBlock()
{
   MqlDateTime gt;
   TimeToStruct(TimeGMT(), gt);
   if(gt.hour < SessionCloseHour || gt.hour >= SessionCloseEndHour)
   ...
   if(spread<=0.0)
      return false;
   bool wide = (spread >= SessionCloseWideSpreadAtr * atr[0]);
   long vol_now = iVolume(_Symbol,PERIOD_H1,0);
   if(vol_now<=0)
      return false;
   double sample[];
   ...
   if(med<=0.0)
      return false;
   bool thin = ((double)vol_now < SessionCloseThinVolRatio * med);
   return wide && thin;
}

bool MondayLondonWideBlock()
{
   MqlDateTime gt;
   TimeToStruct(TimeGMT(), gt);
   if(gt.day_of_week != 1)
