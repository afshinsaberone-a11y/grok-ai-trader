...
//|     شناسه: GRK-XAUUSD-GOLD-DOLLAR-001  |  نسخه: 4.70             |
//+------------------------------------------------------------------+
...
input double FridayLondonThinVolRatio=0.70;
input int    FridayLondonVolLookback=20;
...
int OnInit()
{
   ...
   lastDay=TimeCurrent();
   Print("GRK XAUUSD Gold-Dollar EA v4.70 ready");
...
bool FridayLondonSpreadAtrMedThinBlock()
{
   MqlDateTime gt;
   TimeToStruct(TimeGMT(), gt);
   if(gt.day_of_week != 5)
   ...
   if(gt.hour < LondonOpenHour || gt.hour >= LondonOpenEndHour)
      return false;
   if(!FridayLondonSpreadAtrMedBlock())
      return false;
   long vol_now = iVolume(_Symbol,PERIOD_H1,0);
   if(vol_now<=0)
      return false;
   double sample[];
   ArrayResize(sample, FridayLondonVolLookback);
   int n=0;
   for(int i=1; i<=FridayLondonVolLookback*30 && n<FridayLondonVolLookback; i++)
   {
      datetime bar_time = iTime(_Symbol,PERIOD_H1,i);
      if(bar_time==0)
         continue;
   ...
   if(med<=0.0)
      return false;
   bool thin = ((double)vol_now < FridayLondonThinVolRatio * med);
   return thin;
}
...
bool FridaySessionCloseSpreadAtrMedBlock()
{
   ...
   double ratio_now = spread / atr[0];
   double sample[];
   ArrayResize(sample, FridaySessionCloseSpreadAtrLookback);
   int n=0;
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(point<=0.0)
      return false;
