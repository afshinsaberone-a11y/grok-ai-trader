...
//|     شناسه: GRK-XAUUSD-GOLD-DOLLAR-001  |  نسخه: 2.90             |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   lastDay=TimeCurrent();
   Print("GRK XAUUSD Gold-Dollar EA v2.90 ready");
...
bool WeekendFlattenNow()
{
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt);
   return (dt.day_of_week==5 && dt.hour>=WeekendFlattenHour);
}

bool InSession()
{
   if(!UseSession) return true;
   if(WeekendFlattenNow()) return false;
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt);
...
bool NearSessionEnd()
{
   ...
   if(WeekendFlattenNow()) return true;
...
bool MondayGapBlock()
{
   ...
   if(dt.hour>=MondayGapBlockHour) return false;
   if(MondayGapAtrMult<=0.0) return false;
   double monOpen=iOpen(_Symbol,PERIOD_D1,0);
   double friClose=iClose(_Symbol,PERIOD_D1,1);
   double atr[1];
...
bool PreNewsVolumeBlock()
{
   if(PreNewsVolRatio<=0.0 || PreNewsVolLookback<5) return false;
   if(!PreNewsDay()) return false;
   int n=PreNewsVolLookback;
   double vols[];
   ArrayResize(vols,n);
   int filled=0;
   for(int i=1;i<=n;i++)
   {
      long v=iVolume(_Symbol,PERIOD_D1,i);
      if(v>0)
      {
         vols[filled]=(double)v;
         filled++;
      }
   }
...
bool NyWideSpreadBlock()
{
   MqlDateTime dt; TimeToStruct(TimeCurrent(),dt);
   if(dt.hour<NyOpenHour || dt.hour>=NyOpenEndHour) return false;
   if(NyWideSpreadAtr<=0.0) return false;
   double atr[1];
   if(CopyBuffer(hATR,0,0,1,atr)<1 || atr[0]<=0.0) return false;
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   double spread=ask-bid;
   if(spread<=0.0) return true;
   return (spread>=NyWideSpreadAtr*atr[0]);
}
...
int BarsSinceOpen(datetime openTime)
{
...
void Manage()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(type==POSITION_TYPE_BUY)
      {
         ...
         if(TimeStopBars>0 && !partialDone && profitR<PartialR && BarsSinceOpen(openTime)>=TimeStopBars)
         {
            trade.PositionClose(t);
            continue;
         }
...
void OnTick()
{
   ...
   if(trendUp && squeeze && pullUp && rsi[0]>=RSI_LongLow && rsi[0]<=RSI_LongHigh)
   {
      ...
      if(CostOk(close0-sl, atr[0]))
      {
         ...
         if(lots>0) trade.Buy(lots,_Symbol,0,sl,tp,"XAU-GD-L-v2.9");
      }
   }
   else if(trendDn && squeeze && pullDn && rsi[0]>=RSI_ShortLow && rsi[0]<=RSI_ShortHigh)
   {
      ...
      if(CostOk(sl-close0, atr[0]))
      {
         ...
         if(lots>0) trade.Sell(lots,_Symbol,0,sl,tp,"XAU-GD-S-v2.9");
      }
   }
}
