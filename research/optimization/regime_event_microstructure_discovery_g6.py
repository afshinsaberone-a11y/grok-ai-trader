"""G6 regime-aware event discovery on REAL EURUSD M5.

Discovery 2022-2024; validation 2025; 2026 is never loaded.
Families: regime_momentum and regime_pullback.
"""
...
import numpy as np
import pandas as pd
from research.optimization.cost_aware_gate_v14 import (MIN_EXPECTANCY_R,MIN_PF_EACH_YEAR,MIN_PROFITABLE_YEARS,MIN_TRADES_EACH_YEAR,PRE_OOS_YEARS,PRE_OOS_MAX_DD_PCT,VALIDATION_MAX_DD_PCT,VALIDATION_MIN_PF,VALIDATION_MIN_TRADES)
...
def catalog():
    ...
    assert len(out)==108
    assert len({json.dumps(x,sort_keys=True,separators=(",",":")) for x in out})==108
    return out
...
def prepare(raw):
    ...
    d["ROC3"]=d.Close.pct_change(3); d["ROC6"]=d.Close.pct_change(6); d["ATRq"]=d.ATR.rolling(96,min_periods=48).rank(pct=True); d["day"]=d.index.date
    return d
...
def signal(g,i,p):
    fast=float(g[f"EMA{p['fast']}"].iloc[i]); slow=float(g[f"EMA{p['slow']}"].iloc[i]); pf=float(g[f"EMA{p['fast']}"].iloc[i-1]); ps=float(g[f"EMA{p['slow']}"].iloc[i-1]); aq=float(g.ATRq.iloc[i]); c=float(g.Close.iloc[i]); o=float(g.Open.iloc[i])
    if not np.isfinite(aq) or aq<p["atr_pct"]: return 0
    if p["family"]=="regime_momentum":
        roc=max(abs(float(g.ROC3.iloc[i])),abs(float(g.ROC6.iloc[i]))); return 1 if fast>slow and pf<=ps and roc>0 else -1 if fast<slow and pf>=ps and roc>0 else 0
    ...
    for _,g in d.groupby("day",sort=True):
        ...
        for i in range(50,len(g)-1):
            if pos==0:
                ...
                entry_i=i+1; entry=apply_entry_cost(float(g.Open.iloc[entry_i]),s,cfg); unit=p["atr_mult"]*atr; stop=entry-s*unit; target=entry+s*p["rr"]*unit; pos=s; continue
            age=i-entry_i+1; unit=p["atr_mult"]*float(g.ATR.iloc[entry_i-1])
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--data",required=True); ap.add_argument("--output",required=True); ap.add_argument("--timeframe",required=True,choices=("M1","M5","M15")); ap.add_argument("--spread-pips",type=float,default=.5); ap.add_argument("--slippage-pips",type=float,default=.2); a=ap.parse_args()
    d=prepare(pd.read_csv(a.data)); cfg=ExecutionConfig(spread_pips=a.spread_pips,slippage_pips=a.slippage_pips); years={y:d[(d.index>=f"{y}-01-01")&(d.index<f"{y+1}-01-01")] for y in (2022,2023,2024,2025)}
    rows=[]; gates=Counter()
    ...
    ranked=sorted(rows,key=lambda x:(x["pre_oos_pass"],sum(z["metrics"]["expectancy_R"] for z in x["pre_oos"])),reverse=True); qualified=[x for x in ranked if x["pre_oos_pass"]]; validated=[]
    for x in qualified:
    ...
    for fam in ("regime_momentum","regime_pullback"):
        fr=[x for x in rows if x["family"]==fam]; famsum[fam]={"candidate_count":len(fr),"pre_oos_qualified":sum(x["pre_oos_pass"] for x in fr),"median_trades_by_year":{str(y):float(np.median([z["metrics"]["trades"] for x in fr for z in x["pre_oos"] if z["year"]==y])) for y in PRE_OOS_YEARS},"median_pf_by_year":{str(y):float(np.median([z["metrics"]["profit_factor"] for x in fr for z in x["pre_oos"] if z["year"]==y])) for y in PRE_OOS_YEARS},"median_dd_by_year":{str(y):float(np.median([z["metrics"]["max_dd_pct"] for x in fr for z in x["pre_oos"] if z["year"]==y])) for y in PRE_OOS_YEARS},"median_expectancy_by_year":{str(y):float(np.median([z["metrics"]["expectancy_R"] for x in fr for z in x["pre_oos"] if z["year"]==y])) for y in PRE_OOS_YEARS}}
    payload={"schema_version":"forexai.g6.regime_aware_event.v1","research_timeframe":a.timeframe,"real_data_required":True,"synthetic_fallback":False,"oos_policy":{"loaded":False,"status":"HELD_OUT","start":"2026-01-01"},"execution_model":{"entry":"next_bar_open","cost_pips_per_side":a.spread_pips+a.slippage_pips,"round_trip_cost_pips":2*(a.spread_pips+a.slippage_pips),"same_bar_resolution":"SL first (conservative)","expiry_bars":MAX_HOLD,"overlap":"one position at a time","adverse_exit_cost_applied":True},"catalog":{"candidate_total":len(rows),"family_counts":dict(Counter(x["family"] for x in rows)),"family_summary":famsum},"result":{"candidate_total":len(rows),"pre_oos_qualified_count":len(qualified),"validation_qualified_count":sum(x["validation_pass"] for x in validated),"top_50_diagnostics":ranked[:50],"gate_rejection_counts_all_candidates":dict(gates),"validated_candidates":validated,"champion":None}}
    Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8"); print(json.dumps({"candidate_total":len(rows),"pre_oos_qualified":len(qualified),"validation_qualified":sum(x["validation_pass"] for x in validated),"family_counts":dict(Counter(x["family"] for x in rows))}))
