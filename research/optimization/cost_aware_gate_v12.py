...
def pre_oos_gate(metrics:list[dict[str,Any]])->bool:
    ...
    for y in PRE_OOS_YEARS:
        ...
        if float(m.get('expectancy_R',0.0))<=MIN_EXPECTANCY_R:return False
    return True

def validation_gate(m:dict[str,Any])->bool:
    return _pf(m)>=VALIDATION_MIN_PF and float(m.get('max_dd_pct',100.0))<=VALIDATION_MAX_DD_PCT and int(m.get('trades',0))>=VALIDATION_MIN_TRADES
