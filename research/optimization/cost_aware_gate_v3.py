"""Strict Cost-Aware Gate v3 for pre-OOS candidates.

2022-2024: every year must meet PF/trade floors; at least 2 years positive expectancy.
2025: validation only. 2026: never loaded.
"""
...
def pre_oos_gate(metrics: list[dict[str, Any]]) -> bool:
    ...
    for year in PRE_OOS_YEARS:
        ...
        if int(m.get("trades", 0)) < MIN_TRADES_EACH_YEAR or _pf(m) < MIN_PF_EACH_YEAR:
            return False
