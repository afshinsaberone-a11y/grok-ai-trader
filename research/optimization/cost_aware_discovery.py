...
Uses explicit strict gate v12. 2022-2024 are discovery years; 2025 is validation; 2026 is held out.
"""
...
from research.optimization.cost_aware_gate_v12 import PRE_OOS_YEARS,MIN_PROFITABLE_YEARS,MIN_PF_EACH_YEAR,MIN_EXPECTANCY_R,MIN_TRADES_EACH_YEAR,PRE_OOS_MAX_DD_PCT,VALIDATION_MIN_PF,VALIDATION_MAX_DD_PCT,VALIDATION_MIN_TRADES,candidate_gate,pre_oos_gate
...
def pre_oos_gate_pass(metrics): return bool(pre_oos_gate(metrics))
...
def _pre_oos_reasons(pre):
    reasons=[]; by_year={int(x['year']):x['metrics'] for x in pre}
    ...
    if profitable<MIN_PROFITABLE_YEARS: reasons.append(f'profitable_years<{MIN_PROFITABLE_YEARS}')
    return reasons
def _validation_reasons(m):
    reasons=[]
    if _pf(m)<VALIDATION_MIN_PF: reasons.append(f'validation_pf<{VALIDATION_MIN_PF}')
    if float(m.get('max_dd_pct',100))>VALIDATION_MAX_DD_PCT: reasons.append(f'validation_dd>{VALIDATION_MAX_DD_PCT}')
    if int(m.get('trades',0))<VALIDATION_MIN_TRADES: reasons.append(f'validation_trades<{VALIDATION_MIN_TRADES}')
    return reasons
def discover_family_cost_aware(df,family,*,spread_pips,slippage_pips):
    years={y:df[(df.index>=f'{y}-01-01')&(df.index<f'{y+1}-01-01')] for y in (2022,2023,2024,2025)}; results=[]; reject_counts=Counter(); validation_reject_counts=Counter(); examples=[]
    for j,params in enumerate(catalog()[family]):
        pre=[{'year':y,'metrics':backtest_cost_aware(years[y],family,params,spread_pips=spread_pips,slippage_pips=slippage_pips)} for y in PRE_OOS_YEARS]; reasons=_pre_oos_reasons(pre)
        for reason in reasons: reject_counts[reason]+=1
        results.append({'family':family,'candidate':j+1,'params':params,'cost_aware_robustness_score':score_cost_aware(pre),'pre_oos_gate':{'pass':not reasons,'years':list(PRE_OOS_YEARS),'min_trades_each_year':MIN_TRADES_EACH_YEAR,'min_pf_each_year':MIN_PF_EACH_YEAR,'min_profitable_years':MIN_PROFITABLE_YEARS,'min_expectancy_R':MIN_EXPECTANCY_R,'max_dd_pct':PRE_OOS_MAX_DD_PCT},'pre_oos_cost_aware':pre})
        if reasons and len(examples)<20: examples.append({'candidate':j+1,'reasons':reasons})
    ...
    for x in finalists:
        vm=backtest_cost_aware(years[2025],family,x['params'],spread_pips=spread_pips,slippage_pips=slippage_pips); vr=_validation_reasons(vm)
        for reason in vr: validation_reject_counts[reason]+=1
    ...
    return {'family':family,'candidate_total':len(results),'cost_profile':{'spread_pips':spread_pips,'slippage_pips':slippage_pips,'round_trip_cost_pips':2*(spread_pips+slippage_pips)},'qualified_count':len(qualified),'champion':qualified[0] if qualified else None,'top_50':validated,'reject_diagnostics':{'pre_oos_rejections':dict(sorted(reject_counts.items(),key=lambda x:(-x[1],x[0]))),'pre_oos_rejected_candidates':sum(1 for x in results if not x['pre_oos_gate']['pass']),'pre_oos_passed_candidates':len(finalists),'validation_rejections':dict(sorted(validation_reject_counts.items(),key=lambda x:(-x[1],x[0]))),'validation_rejected_candidates':sum(1 for x in validated if not x['validation_qualifies']),'validation_qualified_candidates':len(qualified),'examples':examples}}
def main():
