"""Cost-aware multi-family discovery v15.
...
def backtest_execution_equivalent(df: pd.DataFrame, family: str, params: dict[str, Any], *, spread_pips=.5, slippage_pips=.2) -> dict[str, Any]:
    ...
    for i in range(1,len(d)):
        a=atr[i-1] if i-1 >= 0 else np.nan
        if pos==0:
            s=sig[i-1]
...
def _score(pre):
    if not pre_oos_gate(pre): return -999.0
    pfs=[_pf(x['metrics']) for x in pre]; total=sum(float(x['metrics']['total_R']) for x in pre); dd=max(float(x['metrics']['max_dd_pct']) for x in pre)
    return round(2.5*min(max(min(pfs),0),2)/2 + 1.5*min(max(sum(pfs)/3,0),2)/2 + 1.5*np.tanh(total/150) - 2.5*min(max(dd,0),100)/100,6)
...
def discover(df,family,spread_pips,slippage_pips):
    ...
    for j,params in enumerate(catalog()[family],1):
        ...
        results.append({'family':family,'candidate':j,'params':params,'cost_aware_robustness_score':_score(pre),'pre_oos_gate':{'pass':not reasons,'years':list(PRE_OOS_YEARS),'fail_closed':True},'pre_oos_cost_aware':pre,'pre_oos_rejection_reasons':reasons})
