...
- sp2l_lite (engineering proxy; not an official SP2L rule set)
"""
...
def indicators(df: pd.DataFrame, p: dict[str, Any]) -> pd.DataFrame:
    ...
    d["ema_f"] = c.ewm(span=p.get("fast",13), adjust=False).mean()
    d["ema_s"] = c.ewm(span=p.get("slow",89), adjust=False).mean()
    d["ema_t"] = c.ewm(span=p.get("trend",200), adjust=False).mean()
    d["mom"] = c.pct_change(p.get("mom",10))
    n=p.get("bb",20); m=c.rolling(n).mean(); s=c.rolling(n).std()
    d["bb_mid"], d["bb_up"], d["bb_dn"] = m, m+p.get("bb_k",2.0)*s, m-p.get("bb_k",2.0)*s
    r=p.get("rsi",14); delta=c.diff(); gain=delta.clip(lower=0).rolling(r).mean(); loss=(-delta.clip(upper=0)).rolling(r).mean(); rs=gain/loss.replace(0,np.nan)
    d["rsi"] = 100-(100/(1+rs))
    d["hh"] = h.rolling(p.get("don",20)).max().shift(1); d["ll"] = l.rolling(p.get("don",20)).min().shift(1)
    d["hour"] = d.index.hour
    d["session"] = ((d.hour>=7)&(d.hour<=16)) | ((d.hour>=12)&(d.hour<=20))
    return d
...
def signal_family(d: pd.DataFrame, family: str, p: dict[str, Any]) -> pd.Series:
    ...
    if family=="trend_ema_rsi":
        sig[((d.ema_f>d.ema_s)&(d.ema_f.shift()<=d.ema_s.shift())&(d.Close>d.ema_t)&(d.rsi>35)&(d.rsi<70)&d.session)]=1
        sig[((d.ema_f<d.ema_s)&(d.ema_f.shift()>=d.ema_s.shift())&(d.Close<d.ema_t)&(d.rsi<65)&(d.rsi>30)&d.session)]=-1
        ...
        day=d.index.floor("D"); prior=(d.hour<7); rh=d.High.where(prior).groupby(day).cummax().shift(1); rl=d.Low.where(prior).groupby(day).cummin().shift(1)
        sig[(d.hour>=7)&(d.Close>rh)]=1; sig[(d.hour>=7)&(d.Close<rl)]=-1
    elif family=="volatility_expansion":
        atr_base=d.atr.rolling(p["vol_lookback"]).mean(); expanding=d.atr>atr_base*p["vol_mult"]
        sig[expanding&(d.Close>d.ema_t)]=1; sig[expanding&(d.Close<d.ema_t)]=-1
    elif family=="sp2l_lite":
        body=(d.Close-d.Open).abs(); rng=d.High-d.Low; spike=(rng>0)&(body/rng>=p["body_ratio"])&(rng>=p["atr_mult"]*d.atr)
        gap_up=d.Low>d.High.shift(2); gap_dn=d.High<d.Low.shift(2)
        sig[spike&gap_up&d.Close>d.Open]=1; sig[spike&gap_dn&d.Close<d.Open]=-1
    return sig


def backtest(df: pd.DataFrame, family: str, p: dict[str, Any]) -> dict[str, Any]:
    d=indicators(df,p); sig=signal_family(d,family,p)
    equity=10000.0; peak=equity; maxdd=0.0; pos=0; entry=stop=tp=0.0; rs=[]
    for i in range(1,len(d)):
        r=d.iloc[i]; atr=r.atr
        if pd.isna(atr) or atr<=0: continue
        if pos==0 and sig.iloc[i]!=0:
            pos=int(sig.iloc[i]); entry=float(r.Close); stop=entry-pos*p["atr_stop"]*atr; tp=entry+pos*p["rr"]*abs(entry-stop); continue
    ...
    n=len(rs); wins=sum(x>0 for x in rs); gp=sum(x for x in rs if x>0); gl=abs(sum(x for x in rs if x<=0)); pf=gp/gl if gl else (float("inf") if gp>0 else 0.0)
    total=sum(rs); return {"trades":n,"win_rate":round(100*wins/n,2) if n else 0.0,"total_R":round(total,2),"expectancy_R":round(total/n,3) if n else 0.0,"profit_factor":round(pf,2) if np.isfinite(pf) else "inf","max_dd_pct":round(100*maxdd,2),"final_equity":round(equity,2)}
...
def score(year_metrics:list[dict[str,Any]])->float:
    ...
    pfs=[3.0 if x["metrics"]["profit_factor"]=="inf" else float(x["metrics"]["profit_factor"]) for x in year_metrics]
    ex=[float(x["metrics"]["expectancy_R"]) for x in year_metrics]; dd=[float(x["metrics"]["max_dd_pct"]) for x in year_metrics]
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument("--data",required=True); ap.add_argument("--output",default="artifacts/multi_family_discovery.json")
    ...
    results=[]
    for fam,params_list in catalog().items():
        for j,p in enumerate(params_list):
            pre=[{"year":y,"metrics":backtest(years[y],fam,p)} for y in (2022,2023,2024)]; s=score(pre); results.append({"family":fam,"candidate":j+1,"params":p,"robustness_score":s,"pre_oos":pre})
    results.sort(key=lambda x:(-x["robustness_score"],x["family"],x["candidate"])); finalists=results[:50]
    final=[]
    for x in finalists:
        vm=backtest(years[2025],x["family"],x["params"]); final.append({**x,"validation_2025":vm,"validation_qualifies":float(vm["profit_factor"] if vm["profit_factor"]!="inf" else 3.0)>=VALIDATION_MIN_PF and float(vm["max_dd_pct"])<=VALIDATION_MAX_DD and int(vm["trades"])>=MIN_TRADES})
    qualified=[x for x in final if x["validation_qualifies"]]; qualified.sort(key=lambda x:(-x["robustness_score"],x["family"],x["candidate"]))
    report={"schema_version":"forexai.multi_family_discovery.v1","families":list(FAMILIES),"candidate_total":len(results),"qualified_count":len(qualified),"champion":qualified[0] if qualified else None,"top_50":final,"oos_policy":{"loaded":False,"start":"2026-01-01","status":"HELD_OUT"},"sp2l_status":"SP2L-lite is an engineering proxy; official SP2L PR #2 is not merged into main"}
    out=Path(args.output); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(_native(report),indent=2,sort_keys=True),encoding="utf-8"); print(json.dumps(_native({"qualified_count":len(qualified),"champion":qualified[0] if qualified else None,"families":list(FAMILIES),"candidate_total":len(results)}),indent=2)); return 0
