"""Targeted Momentum Breakout Discovery v15.3.

Focused real-data M5 research around the strongest v15.2 momentum region.
Strict gates are unchanged. 2022-2024 discovery, 2025 validation, 2026 held out.
Execution-equivalent to v28.1/v29.1: next-bar-open, adverse 1.4 pip round trip,
...
def backtest(df: pd.DataFrame, p: dict[str,Any], *, spread_pips=.5, slippage_pips=.2):
    ...
    # Same momentum-breakout rule family; only the targeted parameter region changes.
    ...
    for i in range(1,len(d)):
        ...
        if pos==0:
            sig_prev=s[i-1]
            if sig_prev==0 or not np.isfinite(a) or a<=0: continue
            pos=int(sig_prev); raw=op[i]; entry=raw+(adverse if pos==1 else -adverse)
            sd=float(p["atr_stop"])*a; stop=entry-pos*sd; tp=entry+pos*float(p["rr"])*sd; entry_i=i; entries+=1; continue
        sig_prev=s[i-1]; trade_r=None
        if pos==1:
            if lo[i] <= stop: trade_r=(stop-entry)/abs(entry-stop)
            elif hi[i] >= tp: trade_r=(tp-entry)/abs(entry-stop)
            elif sig_prev==-1: trade_r=(cl[i]-adverse-entry)/abs(entry-stop)
            elif i-entry_i>=MAX_HOLD_BARS: trade_r=(cl[i]-adverse-entry)/abs(entry-stop)
        else:
            if hi[i] >= stop: trade_r=(entry-stop)/abs(stop-entry)
            elif lo[i] <= tp: trade_r=(entry-tp)/abs(stop-entry)
            elif sig_prev==1: trade_r=(entry-(cl[i]+adverse))/abs(stop-entry)
            elif i-entry_i>=MAX_HOLD_BARS: trade_r=(entry-(cl[i]+adverse))/abs(stop-entry)
        if trade_r is not None:
            rs.append(float(trade_r)); pos=0; exits+=1
            equity*=1+RISK_PCT*trade_r; peak=max(peak,equity); maxdd=max(maxdd,(peak-equity)/peak)
    if pos:
        trade_r=((cl[-1]-adverse-entry)/abs(entry-stop)) if pos==1 else ((entry-(cl[-1]+adverse))/abs(stop-entry))
        rs.append(float(trade_r)); pos=0; exits+=1
        equity*=1+RISK_PCT*trade_r; peak=max(peak,equity); maxdd=max(maxdd,(peak-equity)/peak)
    if entries!=exits or pos!=0: raise SystemExit(f"EXECUTION_INTEGRITY_FAIL: entries={entries} exits={exits} pos={pos}")
    expected=10000.0
    for rr in rs: expected*=1+RISK_PCT*rr
    if not np.isclose(equity,expected,rtol=1e-12,atol=1e-9): raise SystemExit(f"EXECUTION_INTEGRITY_FAIL: equity_mismatch observed={equity} expected={expected}")
...
def catalog():
    ...
    return [{**base,"mom":m,"threshold":th,"atr_stop":a,"rr":rr,"don":n}
            for m in (2,3,4)
            for th in (0.0020,0.0025,0.0030)
            for a in (1.6,1.9,2.2)
...
def reasons(pre):
    ...
    if sum(float(by[y]["expectancy_R"])>MIN_EXPECTANCY_R for y in PRE_OOS_YEARS)<MIN_PROFITABLE_YEARS: out.append(f"profitable_years<{MIN_PROFITABLE_YEARS}")
    return out
...
def discover(df, spread, slippage):
    ...
    finals=[x for x in results if x["pre_oos_gate"]["pass"]][:50]
