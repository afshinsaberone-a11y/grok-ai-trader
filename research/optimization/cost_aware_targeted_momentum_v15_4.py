"""Targeted Momentum Discovery v15.4.

Pre-registered frequency-expansion sweep around the profitable v15.3 region.
No gate relaxation. REAL EURUSD M5 only; 2022-2024 discovery, 2025 validation,
2026 held out. Execution matches v28.1/v29.1.
"""
...
PIP_SIZE=.0001; RISK_PCT=.005; MAX_HOLD_BARS=30; FAMILY="momentum_breakout"
...
def backtest(df:pd.DataFrame,p:dict[str,Any],*,spread_pips=.5,slippage_pips=.2):
    ...
    s=sig.to_numpy(copy=False); op=d.Open.to_numpy(float); hi=d.High.to_numpy(float); lo=d.Low.to_numpy(float); cl=d.Close.to_numpy(float); atr=d.atr.to_numpy(float)
    ...
    for i in range(1,len(d)):
        ...
        if pos==0:
            ...
            pos=int(q); raw=op[i]; entry=raw+(adverse if pos==1 else -adverse); sd=p["atr_stop"]*a
            stop=entry-pos*sd; tp=entry+pos*p["rr"]*sd; entry_i=i; entries+=1; continue
        q=s[i-1]; tr=None
        if pos==1:
        ...
        if tr is not None:
            rs.append(float(tr)); pos=0; exits+=1; equity*=1+RISK_PCT*tr; peak=max(peak,equity); maxdd=max(maxdd,(peak-equity)/peak)
    if pos:
...
def catalog():
    ...
    return [{**base,"mom":m,"threshold":th,"atr_stop":a,"rr":rr,"don":n}
      for m in (1,2,3,4)
      for th in (0.0010,0.0015,0.0020,0.0025,0.0030)
      for a in (1.4,1.6,1.8,2.0)
