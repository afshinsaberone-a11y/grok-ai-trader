"""Strict cost-aware discovery gate v14.
2022-2024 are pre-OOS discovery; 2025 is validation; 2026 is held out.
Fail closed: every pre-OOS year must satisfy all floors, including positive net return.
"""
...
def _positive_return(m:dict[str,Any])->bool:
    return float(m.get("expectancy_R",0.0))>MIN_EXPECTANCY_R and float(m.get("total_R",0.0))>0.0
...
def pre_oos_gate(metrics:list[dict[str,Any]])->bool:
    ...
    if set(by)!=set(PRE_OOS_YEARS): return False
    for y in PRE_OOS_YEARS:
        m=by[y].get("metrics",{})
        if int(m.get("trades",0))<MIN_TRADES_EACH_YEAR: return False
        if _pf(m)<MIN_PF_EACH_YEAR: return False
        if not _positive_return(m): return False
        if float(m.get("max_dd_pct",100.0))>PRE_OOS_MAX_DD_PCT: return False
    return True

def validation_gate(m:dict[str,Any])->bool:
    return (_pf(m)>=VALIDATION_MIN_PF and float(m.get("max_dd_pct",100.0))<=VALIDATION_MAX_DD_PCT and int(m.get("trades",0))>=VALIDATION_MIN_TRADES and _positive_return(m))
