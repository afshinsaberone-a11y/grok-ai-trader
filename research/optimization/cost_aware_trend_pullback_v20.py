"""ForexAI v20: cost-aware trend-following pullback discovery on REAL EURUSD M5.
...
def catalog():
    out=[]
    for fast,slow in ((13,55),(13,89),(21,89)):
      for pull in (0.25,0.5,0.75):
       for stop in (1.5,2.0,2.5):
        for rr in (1.0,1.5,2.0):
         for session in ((0,24),(6,20)):
          for mom in (1,2):
           out.append(dict(ema_fast=fast,ema_slow=slow,pullback_atr=pull,atr_stop=stop,rr=rr,session_start=session[0],session_end=session[1],momentum_bars=mom))
    return out
...
def metrics(rs):
 ...
 return {'trades':n,'win_rate':round(100*wins/n,2) if n else 0.,'total_R':round(float(sum(rs)),2),'expectancy_R':round(float(sum(rs))/n,4) if n else 0.,'profit_factor':round(float(pf),3) if pf!='inf' else pf,'max_dd_pct':round(100*dd,2),'final_equity':round(eq,2),'execution_model':'v28.1_next_bar_open','entries':n,'exits':n,'open_position':False}

def bt(df,p,spread=.5,slippage=.2):
 if len(df)<100:return metrics([])
 d=df.copy(); c=d.Close.astype(float); h=d.High.astype(float); l=d.Low.astype(float)
 d['ef']=c.ewm(span=p['ema_fast'],adjust=False).mean(); d['es']=c.ewm(span=p['ema_slow'],adjust=False).mean()
 tr=pd.concat([h-l,(h-c.shift()).abs(),(l-c.shift()).abs()],axis=1).max(axis=1); d['atr']=tr.rolling(14).mean()
 ef=d.ef.to_numpy(float); es=d.es.to_numpy(float); atr=d.atr.to_numpy(float); op=d.Open.to_numpy(float); hi=h.to_numpy(float); lo=l.to_numpy(float); cl=c.to_numpy(float); ts=d.index
 cost=(spread+slippage)*PIP_SIZE; pos=0; entry=stop=tp=0.; entry_i=-1; rs=[]
 for i in range(1,len(d)):
  if pos==0:
   t=ts[i-1]; hour=t.hour+t.minute/60
   if not(p['session_start']<=hour<p['session_end']) or not np.isfinite(atr[i-1]) or atr[i-1]<=0: continue
   a=atr[i-1]; bullish=ef[i-1]>es[i-1] and cl[i-1]>ef[i-1]; bearish=ef[i-1]<es[i-1] and cl[i-1]<ef[i-1]
   # Pullback: prior close is near/through fast EMA, while trend remains aligned; momentum confirms with a fresh close in trend direction.
   prev_pull_long=(cl[i-1] >= ef[i-1]-p['pullback_atr']*a) and (cl[i-1] <= ef[i-1]+p['pullback_atr']*a)
   prev_pull_short=(cl[i-1] <= ef[i-1]+p['pullback_atr']*a) and (cl[i-1] >= ef[i-1]-p['pullback_atr']*a)
   m=p['momentum_bars']; long_mom=cl[i-1]>cl[i-1-m] if i-1-m>=0 else False; short_mom=cl[i-1]<cl[i-1-m] if i-1-m>=0 else False
   s=1 if bullish and prev_pull_long and long_mom else (-1 if bearish and prev_pull_short and short_mom else 0)
   if not s: continue
   pos=s; entry=op[i]+(cost if s==1 else -cost); dist=p['atr_stop']*a; stop=entry-s*dist; tp=entry+s*p['rr']*dist; entry_i=i; continue
  if pos==1:
   ep=None
   if lo[i]<=stop and hi[i]>=tp: ep=stop
   elif lo[i]<=stop: ep=stop
   elif hi[i]>=tp: ep=tp
   elif ef[i-1]<es[i-1] or cl[i-1]<es[i-1]: ep=cl[i]-cost
...
def main():
 ...
 years={y:d[(d.index>=f'{y}-01-01')&(d.index<f'{y+1}-01-01')] for y in (2022,2023,2024,2025)}; results=[]
 for i,p in enumerate(catalog(),1):
  pre=[{'year':y,'metrics':bt(years[y],p,a.spread_pips,a.slippage_pips)} for y in PRE_OOS_YEARS]; reasons=gate(pre); results.append({'candidate':i,'params':p,'score':score(pre),'pre_oos':pre,'pre_oos_pass':not reasons,'rejection_reasons':reasons})
