...
# Fixed before evaluation. No 2025-based parameter selection.
CATALOG = []
for pivot in (3, 5, 8):
...
def prep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d = d[d.timestamp.dt.year.isin(YEARS)].copy()
...
def pivots(d: pd.DataFrame, p: int):
    # Confirmed pivots only: a pivot at i is known after p bars to its right have closed.
    ph = d.High.eq(d.High.rolling(2*p+1, center=True).max())
    pl = d.Low.eq(d.Low.rolling(2*p+1, center=True).min())
    return ph.fillna(False), pl.fillna(False)


def signals(d: pd.DataFrame, p: dict) -> pd.Series:
    ph, pl = pivots(d, p["pivot"]); out = pd.Series(0, index=d.index, dtype=int)
    highs=[]; lows=[]
    # Signal is emitted only after the pivot confirmation bar.
...
def backtest(d: pd.DataFrame, p: dict) -> dict:
    if len(d) < 100: return {"trades":0,"win_rate":0.0,"total_R":0.0,"expectancy_R":0.0,"profit_factor":0.0,"max_dd_pct":0.0,"entries":0,"exits":0}
    ...
    for i in range(1, len(d)):
        if pos is not None:
            ...
            if slhit or tphit or opposite or age >= MAX_HOLD:
                ...
                r=(ex-pos["entry"])/(pos["entry"]-pos["sl"])*pos["side"]
                rs.append(float(r)); pos=None
        if pos is None and bool(sig.iloc[i-1]):
            ...
            risk=p["atr_mult"]*atr; sl=entry-side*risk; tp=entry+side*p["rr"]*risk
...
def main():
    ...
    raw=pd.read_csv(a.data); d=prep(raw); years={y:d[d.index.year==y] for y in YEARS}; results=[]
    for cid,p in enumerate(CATALOG,1):
        pre={y:backtest(years[y],p) for y in PRE_OOS_YEARS}; reasons=gate(pre)
        score=round(1.5*min(m["profit_factor"] for m in pre.values()) + np.mean([m["expectancy_R"] for m in pre]) + np.tanh(sum(m["total_R"] for m in pre)/100) - max(m["max_dd_pct"] for m in pre)/100,6)
    ...
    print(json.dumps({"candidate_total":len(results),"pre_oos_qualified":len(qualified),"champion":None}))
