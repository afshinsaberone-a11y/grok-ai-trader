...
def backtest(d,p,cfg):
    rs=[]
    # Each local trading date has one deterministic range-building period.
    for local_day,g0 in d.groupby("local_date",sort=True):
        g=g0.copy()
        pos=0; entry=stop=target=0.0; entry_i=-1
        range_end_total=p["entry_hour"]*60
        range_start_total=range_end_total-int(p["range_minutes"])
        for i in range(100,len(g)-1):
            lh=int(g.local_hour.iloc[i]); lm=int(g.local_minute.iloc[i]); minute=lh*60+lm
            if pos==0 and minute==range_end_total:
                bars=g.iloc[:i]
                mins=bars.local_hour*60+bars.local_minute
                ref=bars[(mins>=range_start_total)&(mins<range_end_total)]
                if len(ref)<max(2,int(p["range_minutes"]//5)):
                    continue
                ...
                if side:
                    ...
                    unit=float(p["atr_mult"])*atr
                    stop=entry-side*unit
                    target=entry+side*float(p["rr"])*unit
                    pos=side
                    continue
            if pos!=0:
                age=i-entry_i+1
        ...
        if pos!=0:
            ...
            unit=float(p["atr_mult"])*float(g.ATR.iloc[max(entry_i-1,0)])
...
def reject(pre):
    ...
    if sum(float(by[y]["expectancy_R"])>0 for y in (2022,2023,2024))<MIN_PROFITABLE_YEARS:
...
def main():
    ...
    pre=[x for x in ranked if x["pre_oos_pass"]]
    validated=[]
    for x in pre:
    ...
    payload={
      "schema_version":"forexai.london_breakout_discovery.g3",
      ...
      "catalog":{"candidate_total":len(rows),"family_counts":{"london_breakout":len(rows)}},
      "result":{"candidate_total":len(rows),"pre_oos_qualified_count":len(pre),"validation_qualified_count":sum(1 for x in validated if x["validation_pass"]),"top_50_diagnostics":ranked[:50],"gate_rejection_counts_all_candidates":dict(gates),"validated_candidates":validated,"champion":None},
    }
