...
def main():
    ...
    for p in catalog():
        ...
        for cost in COSTS:
            ...
            rec['costs'][str(cost)]={'years':ys,'positive_years':sum(ys[str(y)]['expectancy_R']>0 for y in YEARS),'sum_expectancy':sum(ys[str(y)]['expectancy_R'] for y in YEARS),'min_pf':min(ys[str(y)]['profit_factor'] for y in YEARS),'max_dd':max(ys[str(y)]['max_dd_pct'] for y in YEARS)}
        rec['validation_2025']={str(c):run_one(years[2025],p,c) for c in COSTS}
        rows.append(rec)
    summary={}
    for cost in COSTS:
        key=str(cost); eligible=[r for r in rows if r['costs'][key]['positive_years']>=2 and r['costs'][key]['min_pf']>=1.05 and r['costs'][key]['max_dd']<=35]
        summary[key]={'candidate_count':len(rows),'diagnostic_robust_count':len(eligible),'best_sum_expectancy':max((r['costs'][key]['sum_expectancy'] for r in rows),default=None),'best_min_pf':max((r['costs'][key]['min_pf'] for r in rows),default=None),'best_max_dd':min((r['costs'][key]['max_dd'] for r in rows),default=None)}
