...
def main():
    ...
    for cost in COSTS:
        key=str(cost)
        eligible=[r for r in rows if r['costs'][key]['strict_pre_oos_pass']]
        summary[key]={
            'candidate_count':len(rows),
            'diagnostic_robust_count':len(eligible),
    ...
    tf_payload={
        ...
        'execution_model':{'entry':'next_bar_open','same_bar_resolution':'SL first (conservative)','expiry_bars':24,'overlap':'one position at a time'},
    ...
    Path(a.output).write_text(json.dumps(tf_payload,ensure_ascii=False,indent=2),encoding='utf-8')
