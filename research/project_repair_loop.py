...
REQUIRED_MARKERS = [
    ("risk", r"0\.5\s*%|RiskPercent"),
    ("spread_filter", r"spread|اسپرد"),
    ("news_filter", r"news|خبر|Calendar"),
    ("daily_loss", r"MaxDailyLoss|ضرر روزانه"),
]
...
def scan_file(path: Path):
    ...
    for pat, rule in FORBIDDEN:
        if re.search(pat, lower):
            if "forbidden" in lower or "ممنوع" in text or "false" in lower or "allowgrid" in lower or "allowmartingale" in lower:
                if rule in {"grid_forbidden", "martingale_forbidden"}:
                    continue
            findings.append(Finding(str(path), rule, pat, "high"))
    if path.suffix == ".mq5" and "GRK_Hybrid_Regime_EA" in path.name:
