...
def audit_ea(text: str, path: Path) -> list[str]:
    ...
    if "version   \"3.17\"" not in text and "version   \"3.17\"" not in text.replace(" ", ""):
        if '#property version   "3.17"' not in text:
            errors.append(f"{path}: champion version should be 3.17")
    return errors
