#!/usr/bin/env python3
"""Static repair loop for GRK EAs. Exit 0 only if all checks pass."""
...
def audit(text: str) -> list[str]:
    fails: list[str] = []
    if "iClose(_Symbol,PERIOD_CURRENT,0)" in text.replace(" ", "") and "iClose(_Symbol,PERIOD_CURRENT,1)" not in text.replace(" ", ""):
    ...
    if "REG_SHOCK" not in text and "SHOCK" not in text:
    ...
    if "SYMBOL_TRADE_STOPS_LEVEL" not in text and "StopsLevel" not in text:
    ...
    if re.search(r"CopyBuffer\s*\(\s*hBB\s*,\s*0\s*,", text) and re.search(r"bbU", text):
        if re.search(r"CopyBuffer\(hBB,0,1,3,bbU\)", text.replace(" ", "")):
