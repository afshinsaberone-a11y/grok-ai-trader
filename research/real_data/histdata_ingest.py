...
def _fetch(url: str, *, timeout: int = 60) -> bytes:
    request = Request(url, headers={"User-Agent": "ForexAI/0.1 real-data-ingestion", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"})
...
def fetch_archive(year: int, output_dir: str | Path, *, month: int | None = None, retries: int = 3, timeout: int = 60) -> DownloadedArchive:
    ...
    page_url = f"{BASE_URL}/download-free-forex-historical-data/?/ascii/1-minute-bar-quotes/eurusd/{year}/{month}" if month is not None else DOWNLOAD_PAGE.format(year=year)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if month is not None:
                fields.update({"date": str(year), "datemonth": f"{year}{month:02d}", "platform": "ASCII", "timeframe": "M1", "fxpair": "EURUSD"})
            request = Request(POST_URL, data=urlencode(fields).encode("utf-8"), headers={"User-Agent": "ForexAI/0.1 real-data-ingestion", "Origin": BASE_URL, "Referer": page_url, "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/zip, application/octet-stream, */*"})
            with urlopen(request, timeout=timeout) as response:
                content = response.read()
            if not content.startswith(b"PK"):
                raise HistDataIngestError(f"HistData returned a non-ZIP response for {year}/{month:02d}" if month is not None else f"HistData returned a non-ZIP response for {year}")
            digest = hashlib.md5(content).hexdigest()
            if fields["tk"].lower() != digest.lower():
                raise HistDataIngestError(f"HistData archive token/hash mismatch for {year}/{month:02d}: token={fields['tk']} md5={digest}" if month is not None else f"HistData archive token/hash mismatch for {year}: token={fields['tk']} md5={digest}")
    ...
    raise HistDataIngestError(f"Failed to download real HistData EURUSD {year}/{month:02d}: {last_error}" if month is not None else f"Failed to download real HistData EURUSD {year}: {last_error}")
...
def _split_duplicate_source_timestamps(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    ...
    conflict_ts: list[pd.Timestamp] = []
    removed = 0
    for timestamp, group in duplicates.groupby("timestamp", sort=False):
        if len(group[compare_columns].drop_duplicates()) > 1:
            conflict_ts.append(timestamp)
        else:
            removed += len(group) - 1
    conflict_df = duplicates[duplicates["timestamp"].isin(conflict_ts)].copy()
    if conflict_ts:
        LOGGER.warning("Found %d unresolved conflicting HistData timestamps; preserving them for cross-feed review", len(conflict_ts))
    clean = df[~df["timestamp"].isin(conflict_ts)].drop_duplicates(subset=["timestamp"], keep="first").reset_index(drop=True)
    return clean, conflict_df.reset_index(drop=True), removed
...
def ingest(start: date, end: date, output_dir: str | Path, *, timeframe: str = "M1") -> dict[str, object]:
    ...
    df, conflict_df, duplicate_rows_removed = _split_duplicate_source_timestamps(df)

    normalized = Path(output_dir) / "normalized"
    normalized.mkdir(parents=True, exist_ok=True)
    dataset_id = f"{start:%Y%m%d}_{(end - timedelta(days=1)):%Y%m%d}"
    conflict_path = normalized / f"EURUSD_M1_{dataset_id}_source_conflicts.csv"
    if not conflict_df.empty:
        conflict_df.to_csv(conflict_path, index=False)
        LOGGER.warning("Wrote unresolved HistData source conflicts to %s", conflict_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print({"duplicate_rows_removed": result["duplicate_rows_removed"], "source_conflicts": str(result["source_conflicts"]) if result["source_conflicts"] else None})
        return 0
